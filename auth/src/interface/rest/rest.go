...
import (
	...
	"efishery/auth/src/interface/rest/handlers"

	"efishery/auth/src/interface/rest/response"
	"efishery/auth/src/interface/rest/route"
...
func makeRoute(
	...
	ch := userHandler.NewUserHandler(respClient, useCases.UserUsecase)

	r.Route("/api", func(r chi.Router) {
		// health check
		r.Mount("/", route.HealthRouter(hh))
		r.Mount("/user", route.UserAppRouter(ch))
